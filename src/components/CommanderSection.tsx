        ...
        <div className="grid lg:grid-cols-2 gap-10 md:gap-16 items-center">
          ...
          >
            <div className="relative w-[280px] h-[360px] sm:w-[320px] sm:h-[400px] md:w-[380px] md:h-[480px]">
              ...
              <motion.div
                className="absolute -bottom-3 left-1/2 -translate-x-1/2 px-4 py-2 md:px-6 md:py-3 rounded-xl md:rounded-2xl glass-strong flex items-center gap-2"
                animate={{ y: [0, -4, 0] }}
                transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
              >
                <Sparkles className="w-3 h-3 md:w-5 md:h-5 text-magic-gold" />
                <div className="flex gap-2 md:gap-3">
                  {credentials.map((cred, i) => (
                    <div key={i} className="flex items-center gap-1">
                      <span className="text-magic-gold">{cred.icon}</span>
                      <span className="text-[10px] md:text-xs font-semibold text-white">{cred.text}</span>
                    </div>
                  ))}
                </div>
              </motion.div>
